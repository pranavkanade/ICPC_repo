/**
 * Created by farzicoder on 9/26/16.
 */
public class SegTreesLazyPropagation {
    public long [] leaf;
    public long [] update;

    int orginSize;
    public SegTreesLazyPropagation(int [] list){
        orginSize = list.length;
        leaf = new long[4*list.length];
        update = new long[4*orginSize];
    ...
    public void update(int begin, int end, int val){
        update(1, 0, orginSize - 1, begin, end, val);
    }
    ...
    public long query(int begin, int end){
        return query(1, 0, orginSize - 1, begin, end);
    }
